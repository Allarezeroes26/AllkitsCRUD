...
const PlaceOrder = () => {
  ...
  return (
    <div className='flex font-paragraph flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      {/* Left Side */}
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            ...
            <div onClick={() => setMethod('visa')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'visa' ? 'bg-zinc-700' : ''} `}></p>
              <img className='h-5 mx-4' src={VISA} />
            </div>
          ...
          <div className="w-full text-end mt-8">
            <button onClick={() => navigate('/orders')} className='bg-black font-display hover:bg-zinc-500 text-white px-16 py-3 text-sm'>Place Order</button>
          </div>
        </div>
      </div>
    </div>
